% Newton-Euler equations for triple pendulum
...
eq1 = simplify(M_O - Hdot_O);

% AMB about A from FBD (II)
M_A = dot(cross(rG2_A, m2*g*i) + cross(rG3_A, m3*g*i),k);
Hdot_A = I_A2*thetaddot2 + I_A3*thetaddot3;
eq2 = simplify(M_A - Hdot_A);

% AMB about B from FBD (III)
M_B = dot(cross(rG3_B, m3*g*i), k);
Hdot_B = I_B3*thetaddot3;
eq3 = simplify(M_B - Hdot_B);

% Solve NE equations for angular accelerations
[A, B] = equationsToMatrix([eq1, eq2, eq3], zdot(4:6));
matlabFunction(A, B, 'File', 'NE_matrices', 'Vars', {z, p});
